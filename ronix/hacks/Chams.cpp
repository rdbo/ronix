...
bool Ronix::Hacks::Chams(IVModelRender *pModelRender, const DrawModelState_t &state, const ModelRenderInfo_t &pInfo, matrix3x4_t *pCustomBoneToWorld)
{
	...
	if (chams_type != ConfigData::CHAMS_TYPE_INVAL) {
		...
		for (size_t i = 0; i < CHAMS_COUNT; ++i) {
			if (DoChams(config->data.chamsData[chams_type][i])) {
				chams_done = true;
				fnDrawModelExecute(pModelRender, state, pInfo, pCustomBoneToWorld);
			}
		}
